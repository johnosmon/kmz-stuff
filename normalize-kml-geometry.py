...
Core pipeline
- Normalize Google-Earth-created structure:
    * Flatten nested <MultiGeometry> (arbitrary depth)
    * Remove empty geometry containers (<MultiGeometry/>; empty <LineString>)
    * Collapse single-child <MultiGeometry> to the contained geometry (e.g., MultiGeometry->LineString)
...
def ensure_single_document_root(kml_root: ET.Element) -> None:
    ...
    If multiple root children exist, wrap them in a new <Document>.
    """
...
# ---------------------------
# GE-structure normalization
# ---------------------------

def _coords_text_present(ls: ET.Element, coords_tag: str) -> bool:
    coords_el = ls.find(coords_tag)
    return coords_el is not None and bool((coords_el.text or "").strip())


def remove_empty_linestrings(root: ET.Element) -> int:
    ns = ns_uri_from_root(root)
    if ns:
        ET.register_namespace("", ns)
    ...
    for ls in list(root.iter(linestring_tag)):
        if not _coords_text_present(ls, coords_tag):
            p = parent.get(ls)
            if p is not None:
                p.remove(ls)
                removed += 1
...
def flatten_multigeometry(root: ET.Element) -> int:
    ns = ns_uri_from_root(root)
    if ns:
        ET.register_namespace("", ns)
    ...
    while changed:
        changed = False
        for mg in list(root.iter(multigeom_tag)):
            nested = [c for c in list(mg) if c.tag == multigeom_tag]
            if not nested:
                continue

            for child_mg in nested:
                insert_at = list(mg).index(child_mg)
                grandkids = list(child_mg)
                mg.remove(child_mg)
                for j, g in enumerate(grandkids):
                    mg.insert(insert_at + j, g)
                removed += 1
                changed = True
    return removed


def remove_empty_multigeometries(root: ET.Element) -> int:
    ns = ns_uri_from_root(root)
    if ns:
        ET.register_namespace("", ns)
...
def collapse_single_child_multigeometry_in_placemarks(root: ET.Element) -> int:
    ns = ns_uri_from_root(root)
    if ns:
        ET.register_namespace("", ns)
    ...
    for pm in list(root.iter(placemark_tag)):
        ...
        if only_child.tag == multigeom_tag:
            continue
        ...
        siblings = list(p)
        try:
            idx = siblings.index(mg)
        except ValueError:
            idx = len(siblings)

        p.remove(mg)
        p.insert(idx, only_child)
        collapses += 1

    return collapses


def normalize_google_earth_kml_structure(root: ET.Element) -> Tuple[int, int, int, int]:
    nested_removed = flatten_multigeometry(root)
    empty_ls_removed = remove_empty_linestrings(root)
    empty_mg_removed_1 = remove_empty_multigeometries(root)
    collapsed = collapse_single_child_multigeometry_in_placemarks(root)
    empty_mg_removed_2 = remove_empty_multigeometries(root)
    return nested_removed, empty_ls_removed, (empty_mg_removed_1 + empty_mg_removed_2), collapsed
...
class Summary:
    input_kind: str
    chosen_kml_name: str
    mg_nested_removed: int
    empty_linestrings_removed: int
    empty_multigeoms_removed: int
    multigeoms_collapsed: int
    lines_before: int
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="normalize-kml-geometry",
        description="Normalize Google Earth KML/KMZ MultiGeometry LineStrings for QGIS/Google Earth (structure normalize + explode + clean).",
        add_help=True,
    )
...
def main(argv: List[str]) -> int:
    ...
    try:
        ...
        mg_nested_removed, empty_ls_removed, empty_mg_removed, mg_collapsed = normalize_google_earth_kml_structure(root)
        ...
        if args.verbose:
            s = Summary(
                ...
                mg_nested_removed=mg_nested_removed,
                empty_linestrings_removed=empty_ls_removed,
                empty_multigeoms_removed=empty_mg_removed,
                multigeoms_collapsed=mg_collapsed,
            ...
            print(
                "\n".join([
                    ...
                    f"[normalize] nested MultiGeometry removed: {s.mg_nested_removed}",
                    f"[normalize] empty LineStrings removed: {s.empty_linestrings_removed}",
                    f"[normalize] empty MultiGeometry removed: {s.empty_multigeoms_removed}",
                    f"[normalize] MultiGeometry collapsed (single-child): {s.multigeoms_collapsed}",
                    f"[info] LineStrings before explode: {s.lines_before}",
